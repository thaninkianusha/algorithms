...
class Solution:
    ...
    def seats(self, A):
        MOD = 10000003
        position = []  ## stores the position of the occupied seats        
        for i in range(0,len(A)):
            if A[i] == 'x':
                position.append(i)
        median = position[int(len(position)/2)]
        sumi = 0
        k = 1
        for i in range(int(len(position)/2)+1,len(position)):
            sumi = sumi+abs(median+k-position[i])
        
        k = 0
        for i in range(int(len(position)/2)+1,-1):
            sumi = sumi+(median-k-position[i])
        return (sumi%MOD)
